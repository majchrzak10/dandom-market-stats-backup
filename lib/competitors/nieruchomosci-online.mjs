...
// NO interpretuje slug miasta luzno - "Rogozno" w query laczy oferty z roznych
// "Rogoznow" w Polsce (lubelskie, mazowieckie, etc). Filtr na wojewodztwo jest
// niezbedny zeby zostac przy faktycznym regionie Dan-Dom.
const ALLOWED_PROVINCES = new Set(["wielkopolskie"]);
...
export async function fetchNoForCity({ category, citySlug, maxPages = 8 }) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    for (const r of raw) {
      ...
      // ID dodajemy zawsze - zeby paginacja sie skonczyla nawet jak strona
      // zwroci same odfiltrowane.
      seenIds.add(n.externalId);
      // Odrzucamy oferty spoza dozwolonego wojewodztwa (NO laczy miejscowosci
      // o tej samej nazwie z roznych regionow).
      if (n.province && !ALLOWED_PROVINCES.has(n.province.toLowerCase())) {
        pageFiltered++;
        continue;
      }
      offers.push(n);
    ...
    if (pageFiltered > 0) console.log(`[NO] ${citySlug}/${category.slug} p.${page}: filtr wojewodztwa odrzucil ${pageFiltered}`);

    console.log(`[NO] ${citySlug}/${category.slug} p.${page}: ${raw.length} raw, ${pageNew} nowych`);

    // Przerwij dopiero gdy strona zwraca SAME już-widziane ID (paginacja się skończyła).
    // Sam filtr województwa nie kończy paginacji - kolejne strony mogą mieć właściwy region.
    if (pageNew + pageFiltered === 0) break;
